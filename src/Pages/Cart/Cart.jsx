...
const Cart = ({ cartAllProduct = [], setCartAllProduct }) => {
  ...
  useEffect(() => {
    setCounts(cartAllProduct.reduce((acc, product) => ({ ...acc, [product._id]: 1 }), {}));
  }, [cartAllProduct]);
  ...
  const handleDeleteCart = (_id) => {
    const toastId = toast.loading("Removing product...", { duration: 30000 });

    setTimeout(() => {
      toast.dismiss(toastId);
      toast.success('Product removed successfully', { duration: 30000 });
      setCartAllProduct(cartAllProduct.filter((item) => item._id !== _id));
    }, 2000); // Simulating API call delay
  };

  const getTotalPrice = () => {
    return cartAllProduct.reduce((total, product) => {
      return total + (product.price * (counts[product._id] || 1));
    }, 0);
  };
          ...
          {cartAllProduct?.map((product) => (
            <div className="flex flex-col md:flex-row items-center justify-between border-b pb-4" key={product._id}>
              ...
              <div className="flex items-center space-x-2 mt-4 md:mt-0">
                ...
                <input type="number" className="w-12 text-center border rounded-md" value={counts[product._id] ?? 1} readOnly />
                <button onClick={() => increment(product)} className="px-3 py-1 bg-gray-200 rounded-md">+</button>
              </div>
              <p className="text-lg font-semibold mt-4 md:mt-0">${((counts[product._id] ?? 1) * product.price).toFixed(2)}</p>
